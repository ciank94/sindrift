...
fpath = FileExplorer(node="remote", model_name="cmems", key="SOIN")
...
for year_i in year_list:
    date_init = datetime.datetime(year_i, 11, 1, 0, 0)  # beginning of first simulation;
    ...
    time_step = datetime.timedelta(
        hours=1 / 6
    )  # simulation time step (negative time is backwards stepping of model)
    save_step = datetime.timedelta(hours=4)  # save time step
    release_n = 1  # total number of releases for simulation
    date_limit = (
        date_init + (release_step * (release_n - 1)) + duration_days
    )  # final date that will be accessed
    phys_states = fpath.get_phys_states(
        date_init, date_limit
    )  # input netcdf file with physics (u, v ,T ...)

    # load readers before the main loop of the simulation:
    reader_phys_states = reader_netCDF_CF_generic.Reader(
        phys_states
    )  # read input variables
    print(reader_phys_states)
    reader_landmask = (
        reader_global_landmask.Reader()
    )  # high resolution coast for particle beaching etc.
