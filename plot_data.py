...
class PlotData:
    def __init__(self, key, year, compile_folder, analysis_folder):
        ...
        self.analysis_file = (
            self.analysis_folder + self.file_prefix + "R1_trajectory_analysis.nc"
        )
    ...
    def plot_dom_paths(self, release_n):
        ...
        self.plot1 = plt.pcolormesh(
            self.df["lon_bin_vals"][:],
            self.df["lat_bin_vals"][:],
            dom_paths.T,
            cmap=self.dom_cmap,
    ...
    def plot_site_recruit_t(self, release_n):
        ...
        print(
            ...
            + self.year
            + ": "
            + str(np.mean(r_table.recruit_number * 100 / 10000))
        )
        print(
            "recruit number std "
            + self.year
            + ": "
            + str(np.std(r_table.recruit_number * 100 / 10000))
        )
        print(
            "recruit time mean "
            + self.year
            + ": "
            + str(np.mean(r_table.recruit_time / 24))
        )
        print(
            "recruit time std "
            + self.year
            + ": "
            + str(np.std(r_table.recruit_time / 24))
        )
        self.ax.scatter(
            self.df.variables["lon_init"][:],
            self.df.variables["lat_init"][:],
            s=10,
            facecolor="none",
            edgecolors="gray",
            alpha=0.3,
    ...
    def plot_CG_paths(self, df):
        ...
        [
            self.ax.plot(
                df.CG[:, 0, i], df.CG[:, 1, i], alpha=0.9, linewidth=0.5, c="k"
            )
            for i in range(0, np.shape(df.CG)[2])
        ]
    ...
    def plot_recruits(self):
        ...
        [
            ...
            for i in range(0, np.shape(id1)[1])
        ]
        self.ax.scatter(x[:, 0], y[:, 0], alpha=0.7, c="r", s=1)
        x, y = self.poly.exterior.xy
        self.ax.plot(
            x, y, color="y", alpha=0.9, linewidth=2, solid_capstyle="round", zorder=2
        )
    ...
    def get_polygon(self, poly_n):
        ...
        coords = (
            (self.lon_1, self.lat_1),
            (self.lon_1, self.lat_2),
            (self.lon_2, self.lat_2),
            (self.lon_2, self.lat_1),
            (self.lon_1, self.lat_1),
        )  # tuple item;
        poly = Polygon(coords)
        return poly
    ...
    def plot_trajectory(self):
        ...
        step_v2 = np.floor(
            np.shape(self.lon)[1] / (np.shape(self.lon)[1] * 0.99)
        ).astype(int)
    ...
    def plot_background(self, background, ax_name):
        ...
        if background == "BSSI":
            ...
            ax_name.set_extent([self.min_lon, self.max_lon, self.min_lat, self.max_lat])
            # self.ax.set_extent(
            ...
            lon_lim = [-39.4, -35]
            lat_lim = [-55, -53.2]
            ax_name.set_extent([lon_lim[0], lon_lim[1], lat_lim[0], lat_lim[1]])
        else:
            self.gen_lon_lat_extent()
            ax_name.set_extent([self.min_lon, self.max_lon, self.min_lat, self.max_lat])
        return
...
class CatchData:
    ...
    def catch_facts(self):
        ...
        print(
            "CV_catch_AP:  "
            + str(
                (np.nanstd(self.df.krill_greenweight_kg) / 1000)
    ...
    def plot_lat_lon(self):
        ...
        axs[0, 0].hist(
            lon, facecolor="red", edgecolor="gray", linewidth=0.5, alpha=0.75, bins=30
        )
        axs[0, 1].hist(
            lat, facecolor="red", edgecolor="gray", linewidth=0.5, alpha=0.75, bins=30
        )
        axs[0, 2].hist(
            g_depth,
            facecolor="red",
        ...
        axs[1, 0].hist(
            lon, facecolor="blue", edgecolor="gray", linewidth=0.5, alpha=0.75, bins=30
        )
        axs[1, 1].hist(
            lat, facecolor="blue", edgecolor="gray", linewidth=0.5, alpha=0.75, bins=30
        )
        axs[1, 2].hist(
            g_depth,
            facecolor="blue",
        ...
        axs[2, 0].hist(
            lon, facecolor="olive", edgecolor="gray", linewidth=0.5, alpha=0.75, bins=30
        )
        axs[2, 1].hist(
            lat, facecolor="olive", edgecolor="gray", linewidth=0.5, alpha=0.75, bins=30
        )
        axs[2, 2].hist(
            g_depth,
            facecolor="olive",
    ...
    def plot_fishing_season(self):
        ...
        for i in range(1, 13):
            ...
            ap_sum[c] = np.nansum(self.df.krill_greenweight_kg[self.month == i]) / (
                1000 * 1000
            )
        ...
        for i in range(1, 13):
            ...
            so_sum[c] = np.nansum(self.df.krill_greenweight_kg[self.month == i]) / (
                1000 * 1000
            )
        ...
        for i in range(1, 13):
            ...
            sg_sum[c] = np.nansum(self.df.krill_greenweight_kg[self.month == i]) / (
                1000 * 1000
            )
        ...
        for i in range(2006, 2024):
            ...
            ap_sum[c] = np.nansum(self.df.krill_greenweight_kg[self.year_c == i]) / (
                1000 * 1000
            )
        ...
        for i in range(2006, 2024):
            ...
            so_sum[c] = np.nansum(self.df.krill_greenweight_kg[self.year_c == i]) / (
                1000 * 1000
            )
        ...
        for i in range(2006, 2024):
            ...
            sg_sum[c] = np.nansum(self.df.krill_greenweight_kg[self.year_c == i]) / (
                1000 * 1000
            )
...
def plot_poster_dom_paths(compile_folder, analysis_folder):
    ...
    # transform=ccrs.PlateCarree(), extend='both')
    d_map = axs.contourf(
        p_plot.df["lon_bin_vals"][:],
        p_plot.df["lat_bin_vals"][:],
        dom_paths.T,
...
def plot_SOIN_recruit_dom_paths(compile_folder, analysis_folder):
    ...
    for y in years:
        ...
        axs[idy, idx].set_extent([-64, -33, -72, -49])
        filename = compile_folder + p_plot.file_prefix + "dom_paths.npy"
        dom_paths = np.load(filename)
        dom_paths = dom_paths.astype(float)
        # dom_paths[dom_paths == 0] = np.nan
...
def plot_retain(compile_folder, analysis_folder):
    ...
    ax1.bar(np.arange(0, np.shape(flush_time)[0]), flush_time, color="r", alpha=0.75)
    ax1.set_ylabel("time (days)", color="r", fontsize=15)
    ax1.xaxis.set_tick_params(labelsize=14)
    ax1.yaxis.set_tick_params(labelsize=14)
    uniq_years = np.unique(years)
    plt.xticks(
        np.arange(0, np.shape(uniq_years)[0]),
        [
            "2006",
            "2007",
            "2008",
...
def plot_linreg(compile_folder, analysis_folder):
    ...
    for y in years:
        ...
        lat_mid[counter] = np.sum(r_table[2, :] * r_table[1, :]) / np.nansum(
            r_table[2, :]
        )
        lon_mid[counter] = np.nansum(r_table[2, :] * r_table[0, :]) / np.nansum(
            r_table[2, :]
        )
        catch_v[counter] = p_plot.read_catch()
        catch_v_ap[counter], catch_v_so[counter], catch_v[counter] = (
            p_plot.read_prev_catch()
        )
        recruit_v[counter] = p_plot.get_recruits()
    ...
    d_map = ax1[0, 0].scatter(
        varx, vary, c=c_valss, s=m_size, cmap=c_mapt, edgecolors="k"
    )
    ax1[0, 0].plot(
        varx,
...
def plot_recruit_stat(compile_folder, analysis_folder):
    ...
    ax1[0, 0].plot(
        np.arange(0, np.shape(recruit_t)[0]), recruit_t, color="r", linewidth=4
    )
    ...
    ax1[0, 1].plot(
        np.arange(0, np.shape(recruit_v)[0]), recruit_v, color="r", linewidth=4
    )
    ...
    ax1[1, 0].plot(
        np.arange(0, np.shape(recruit_t)[0]), recruit_t, color="b", linewidth=4
    )
    ...
    ax1[1, 1].plot(
        np.arange(0, np.shape(recruit_v)[0]), recruit_v, color="b", linewidth=4
    )
...
def plot_temp_SG(compile_folder, analysis_folder):
    ...
    for y in range(2006, 2020 + 1):
        ...
        id_y = cdata.year_c == y
        catches[counter] = (
            np.nanmean(cdata.df.krill_greenweight_kg[id_lat_c & id_lon_c & id_y]) / 1000
        )
        id_time = year == y
    ...
    for m in range(6, 9 + 1):
        ...
        id_m = cdata.month == m
        catches[counter] = (
            np.nanmean(cdata.df.krill_greenweight_kg[id_lat_c & id_lon_c & id_m]) / 1000
        )
        id_time = month == m
        vals = theta_box[id_time]
        temp[counter] = np.nanmean(vals)
    ...
    # ax1.set_ylim([0, 14])
    ax2.yaxis.set_tick_params(labelsize=14)
    plt.xticks(np.arange(1, 5), ["Jun", "Jul", "Aug", "Sep"])
    plt.grid(alpha=0.45)  # nice and clean grid
    varx = temp
    vary = catches
    mask = ~np.isnan(varx) & ~np.isnan(vary)
...
def plot_catch_points(compile_folder, analysis_folder):
    ...
    b_map = ax_name.contourf(
        bath_lon,
        bath_lat,
        bath,
        levels=bath_contours,
        transform=ccrs.PlateCarree(),
    ...
    ax_name.set_extent([-64, -33, -72, -49])
...
def plot_seasonal_rec(compile_folder):
    ...
    ax[0, 0].fill_between(df.date, (y - ci), (y + ci), color="r", alpha=0.1)
    ...
    ax[1, 0].fill_between(df.date, (y - ci), (y + ci), color="r", alpha=0.1)
    ...
    ax[0, 1].fill_between(df.date, (y - ci), (y + ci), color="r", alpha=0.1)
    ...
    ax[1, 1].fill_between(df.date, (y - ci), (y + ci), color="r", alpha=0.1)
...
def plot_particles(compile_folder, analysis_folder, trajectory_folder, phys_folder):
    ...
    for y in range(2019, 2020):
        ...
        ax_name[0, 0].quiver(
            phys_file["longitude"][::sk],
            phys_file["latitude"][::sk],
            uu0[::sk, ::sk],
            vv0[::sk, ::sk],
            edgecolors="k",
            alpha=0.7,
        ...
        for y in range(2019, 2020):
            ...
            trajectory_file = (
                trajectory_folder + p_plot.file_prefix + "R10_trajectory.nc"
            )
            nc_file = nc.Dataset(trajectory_file)
...
def plot_worms(compile_folder, analysis_folder, trajectory_folder):
    ...
    ax_name[0, 0].set_extent([-64, -34, -70, -50])
    dens_f[dens_f == 0] = np.nan
    ...
    ax_name[1, 0].set_extent([-64, -34, -70, -50])
    dens_dom[dens_dom == 0] = np.nan
    dens_dom = (dens_dom / (15 * 10000 * 20)) * 100

    # [ax_name[0].plot(lon[i, :], lat[i,:], color='r', linewidth=2, alpha=0.01, markersize=0.1) for i in range(0, lon.shape[0])]
    d_map = ax_name[1, 0].pcolormesh(
        lon_bin_vals,
        lat_bin_vals,
        dens_dom.T,
    ...
    # ax_name[0].contourf(bath_lon, bath_lat, bath, levels=bath_contours,
    #                   transform=ccrs.PlateCarree(), cmap=plt.get_cmap('Blues'), vmin=0, vmax=4000)
    ax_name[0, 1].set_extent([-64, -34, -70, -50])
    dens_f[dens_f == 0] = np.nan
    dens_f = (dens_f / np.nansum(dens_f)) * 100
    ...
    ax_name[1, 1].set_extent([-64, -34, -70, -50])
    dens_dom[dens_dom == 0] = np.nan
    dens_dom = (dens_dom / (15 * 10000 * 20)) * 100
