...
class PostProcess:
    ...
    def initialize_variables(self):
        ...
        self.dom_paths = np.zeros(
            [np.shape(self.lon_bin_vals)[0], np.shape(self.lat_bin_vals)[0]]
        )
        ...
        if self.analysis_df.bounds == "NEMO":
            self.analysis_df.variables["recruit_SG_north"][:] = (
                0  # initialize recruitment to SG north
            )
        elif self.analysis_df.bounds == "SGret":
            self.analysis_df.variables["retain_SG_north"][:] = (
                0  # initialize recruitment to SG north
            )
        return
        ...
        if self.analysis_df.bounds == "NEMO":
            for p_i in range(0, self.shp_p, self.p_stride):
                ...
                print(
                    ...
                    + " of "
                    + str(self.shp_p)
                )  # print particle id number in analysis
                ...
                if self.visit_index[self.p_i] > 0:
                    ...
                    self.recruit_dom_paths[
                        self.id_lon[p_i, range_v], self.id_lat[p_i, range_v]
                    ] = (
                        self.recruit_dom_paths[
                            self.id_lon[p_i, range_v], self.id_lat[p_i, range_v]
                        ]
    ...
    def retain_SG_north(self, lon1, lat1):
        ...
        if not np.sum(idx_retain) == 0:
            ...
            self.transit_hours[self.p_i] = (t_to_poly.days * 24) + np.floor(
                t_to_poly.seconds * 1 / (60 * 60)
            ).astype(int)
    ...
    def read_bio(self):
        self.bio_file = (
            self.fpath.phys_states_path
            + "CMEMS_GLBIO_D_full_"
            + str(self.analysis_df.sim_start_year)
            + ".nc"
        )
        self.bio_states = nc.Dataset(self.bio_file, mode="r")
        self.bio_times = num2date(
            self.bio_states["time"], self.bio_states["time"].units
        )
    ...
    def init_ncfile(self):
        ...
        if self.analysis_df.bounds == "NEMO":
            variable_key_dict = {
                "dom_paths": {
                    "datatype": "i4",
                    "dimensions": ("n_lon_bins", "n_lat_bins"),
                ...
                "recruit_SG_north": {
                    "datatype": "i4",
                    "dimensions": ("trajectory", "transit_info"),
        ...
        for variable in variable_key_dict:
            if variable not in self.analysis_df.variables.keys():
                self.analysis_df.createVariable(
                    variable,
                    variable_key_dict[variable]["datatype"],
                    variable_key_dict[variable]["dimensions"],
                )
                self.analysis_df[variable].description = variable_key_dict[variable][
                    "description"
                ]
        return
